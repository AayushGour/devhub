  ...
  return (
    <Dialog.Root open={open} onOpenChange={v => !v && onClose()}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-[43.75rem] max-h-[82vh] flex flex-col bg-surface border border-border rounded-[1.12rem] shadow-[0_1.5rem_4rem_rgba(0,0,0,0.3)] z-[51] overflow-hidden">
          ...
          <div className="flex gap-0.5 px-6 pt-[0.88rem] border-b border-border shrink-0 overflow-x-auto">
            {TYPES.map(t => (
              <button
                key={t}
                onClick={() => setFilter(t)}
                className={cn(
                  'px-[0.75rem] py-1.5 rounded-t-lg border-none cursor-pointer font-[inherit] text-[0.75rem] tracking-[-0.01rem] border-b-2 -mb-px whitespace-nowrap',
                  filter === t
            ...
            <div className="grid grid-cols-2 gap-[0.62rem]">
              {filtered.map(template => (
                <button
                  ...
                  className={cn(
                    'flex flex-col gap-2 p-4 rounded-[0.88rem] border border-border bg-transparent cursor-pointer font-[inherit] text-left',
                    'hover:border-accent hover:bg-surface-raised transition-[border-color,background-color] duration-150'
                  )}
                >
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-[0.81rem] font-semibold text-on-surface tracking-[-0.01rem]">
                      {template.label}
                    </p>
                    <span className="text-[0.62rem] font-medium text-on-surface-muted bg-surface-raised border border-border rounded-full px-2 py-0.5 shrink-0">
                      {DIAGRAM_TYPE_LABELS[template.type]}
                    </span>
                  </div>
                  <p className="text-[0.75rem] text-on-surface-muted leading-[1.4]">
                    {template.description}
                  </p>
                  <pre className="text-[0.62rem] text-on-surface-muted/50 font-mono overflow-hidden leading-relaxed mt-1">
                    {template.code.split('\n').slice(0, 3).join('\n')}
                  </pre>
                </button>
              ))}
